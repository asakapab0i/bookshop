             ...
             <div class="col-md-12">
                    <div class="panel panel-default" id="panels">
                        ...
                        <div class="panel-body">
                         ...
                         <div class="col-md-6">
                                <div class="panel panel-default" id="panels">
                                <div class="panel-heading">Order # {order_id}

<select class="pull-right"  id="admin_action">
			...
			<option value="Claimed"> to Claimed</option>
			<option value="Unclaimed"> to Unclaimed</option>
                    </select>
                 ...
                 <p>
                 Order Status: <span class="order_status">{order_status}</span>
                 </p>
		<p>Package Status: <span class="package_status">{package_status}</span></p>
...
<script type="text/javascript">
  $(document).on('change', '#admin_action', function(event) {
	...
	//check if status if for package
	if(status == 'Claimed' || status == 'Unclaimed'){
	 $.ajax({
      		url: '<?php echo site_url("staff/post_change_package_status") ?>',
      		type: 'POST',
       	        data: {status: status, order_id:order_id},
     		 })
      		.done(function(e) {
        		$('.package_status').html(e);
      		});
		
	}else{
		 $.ajax({
     		 url: '<?php echo site_url("staff/post_change_status") ?>',
      		type: 'POST',
        		data: {status: status, order_id:order_id},
      		})
      		.done(function(e) {
